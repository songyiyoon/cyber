...
let imgTotal=1000;
let current = 0
let progressTimer;//매시간마다 바뀌는
let topValue;


//setinterval(할일,시간)
//setinterval(function(){},1000) 매 1초 마다 마다 할일

// 
//setinterval 을 멈추고 싶을때 - 변수에 setinterval 할당하기 let 변수 = setInterval
//-clearInterval(변수)을 넣어야한다.


progressTimer=setInterval(updateProgress,1000/60)//function이 너무길면 따로빼서 적어도된다.(펑션이름,시간)이렇게 해줘야함
...
function updateProgress(){
 ...
 progressText.innerHTML= Math.floor(current) + '%';//Math.ceil(소수점 올리기) .floor(버리는거)
...
window.addEventListener("resize",function(){//화면의 사이즈가 달라질때 일어나는일
...
gsap.to(imagesSeq,{
  ...
  snap:"frame",//frame은 프레임단위로 값을 맞추겠다 
  ease:"none",
  scrollTrigger:{
    scrub:0.15,
    trigger:"#page canvas",
  ...
  onUpdate:render,//gsap.to가 변할때 마다(업데이트할떄마다) render는 함수!
})
...
function scaleImage(img,ctx){
  ...
  let hRatio = canvas.width / img.width;//수평
  let vRatio = canvas.height / img.height;//수직
  let ratio=Math.max(hRatio,vRatio)//둘중 큰거를 찾아라max(값1,값2) 둘중 작은거를 찾아라 min(값1,값2)
  let centershift_x=(canvas.width - img.width*ratio)/2//-250
  let centershift_y=(canvas.height - img.height*ratio)/2//0
  
  ctx.clearRect(0, 0, canvas.width,canvas.height);// 이미지지우기
  //이미지그리기
  ctx.drawImage(
    img,
    0,
